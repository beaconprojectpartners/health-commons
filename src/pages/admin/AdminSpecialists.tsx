    ...
    <div className="flex min-h-screen flex-col">
      ...
      <section className="flex-1 py-10">
        <div className="container mx-auto max-w-4xl px-4">
          <h1 className="font-heading text-3xl text-foreground mb-4">Specialist applications</h1>
          <div className="mb-6 flex flex-wrap gap-2">
            <Button
              size="sm"
              variant="outline"
              onClick={async () => {
                const { data, error } = await supabase.functions.invoke("tier-promotion-check");
                if (error) return toast({ title: "Promotion check failed", description: error.message, variant: "destructive" });
                toast({ title: "Promotion check complete", description: `Checked ${data?.checked ?? 0}, promoted ${data?.promoted ?? 0}` });
              }}
            >
            ...
            <Button
              ...
              variant="outline"
              onClick={async () => {
                const { data, error } = await supabase.functions.invoke("npi-reverify");
                if (error) return toast({ title: "NPI re-verify failed", description: error.message, variant: "destructive" });
                toast({ title: "NPI re-verify complete", description: `Checked ${data?.checked ?? 0} of ${data?.considered ?? 0}` });
              }}
            >
              Run NPI re-verification
            </Button>
          </div>
